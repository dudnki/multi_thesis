...
def train_mlp_on_prototypes(C_bert_protos, IMG_protos, epochs=300, lr=1e-3):
    """
    클래스 프로토타입만 사용하여 MLP 프로젝터를 학습시킵니다.
    """
    # 1. 모델, 옵티마이저, 손실 함수 정의
    text_dim = C_bert_protos.shape[1]
    image_dim = IMG_protos.shape[1]
    
    projector = MLPProjector(text_dim, image_dim).to(DEVICE)
    optimizer = torch.optim.AdamW(projector.parameters(), lr=lr)
    loss_fn = nn.MSELoss()

    # 2. 학습 데이터 준비 (프로토타입들을 DEVICE로 이동)
    text_protos_device = C_bert_protos.to(DEVICE)
    image_protos_device = IMG_protos.to(DEVICE)

    # 3. 학습 루프
    projector.train()
    print("\n--- Training MLP Projector on Prototypes ---")
    for epoch in range(1, epochs + 1):
        optimizer.zero_grad()
        
        projected_text_protos = projector(text_protos_device)
        loss = loss_fn(projected_text_protos, image_protos_device)
...
def main():
    ...
    # 5) BERT -> 이미지 공간 정렬행렬 학습 (또는 MLP 프로젝터 학습)
    # 이제 이 부분은 AttentionFusionModel 내에서 처리되거나, 
    # C_map을 단순히 텍스트 특징으로 활용하는 것으로 대체됩니다.
    # C_map은 BERT 텍스트 특징을 이미지 특징 공간으로 이미 정렬한 상태를 가정합니다.
    # 여기서는 C_map을 직접 텍스트 특징으로 사용하겠습니다.
    projector = train_mlp_on_prototypes(C_bert, IMG_protos)
    ...
    for epoch in range(1, 201): # 에폭 수 조절
        fusion_model.train()
        train_loss = 0
