...
def main():
    ...
    M = fit_align_BERT_to_IMG(C_bert, IMG_protos, ridge_lambda=RIDGE_LAMBDA) # 기존 Ridge 정렬 사용
    C_map = F.normalize(C_bert @ M.T, dim=-1) # (K, d_img) BERT 특징을 이미지 공간으로 매핑한 텍스트 프로토타입
    ...
    for epoch in range(1, 201): # 에폭 수 조절
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(fusion_model.state_dict(), "save_model/best_fusion_model.pth")
